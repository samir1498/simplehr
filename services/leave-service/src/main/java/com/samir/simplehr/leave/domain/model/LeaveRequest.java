...
public final class LeaveRequest {
    ...
    public LeaveRequest approve(String reviewerId) {
        ...
        if (status == LeaveStatus.REJECTED) {
            throw new IllegalStateException(
                "cannot approve a rejected leave request"
            );
        }
        return toBuilder()
    ...
    public LeaveRequest reject(String reviewerId, String reason) {
        ...
        if (status == LeaveStatus.APPROVED) {
            throw new IllegalStateException(
                "cannot reject an approved leave request"
            );
        }
        return toBuilder()
    ...
    private static LocalDate requireEndDate(
        LocalDate endDate,
        LocalDate startDate
    ) {
        if (endDate == null) {
        ...
        if (startDate != null && startDate.getYear() != endDate.getYear()) {
            throw new IllegalArgumentException(
                "leave request must be within the same calendar year"
            );
        }
        return endDate;
    }
